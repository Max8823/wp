...
// checking movie codes against what was passed iin POST
  foreach($movies as $movie){
    ...
    function setSession(){
     ...
     $booking = unserialize($tmp);
     $_SESSION['booking_time'] = $booking[0];
      $_SESSION['Name'] = $booking[1];
      $_SESSION['Phone'] = $booking[2];
      $_SESSION['Email'] = $booking[3];
      $_SESSION['movie_code'] = trim($booking[4]);
      $_SESSION['day'] = $booking[5];
      $_SESSION['time'] = $booking[6];
      $_SESSION['seats']=array("STA"=>$booking[7], "STP"=>$booking[8], "STC"=>$booking[9], "FCA"=>$booking[10], "FCP"=>$booking[11], "FCC"=>$booking[12]);
      foreach($_SESSION['seats'] as $key => $value){
        if($value == 0){
          unset($_SESSION['seats'][$key]);
        }
      }
      $_SESSION['Price'] = $booking[13];
      $_SESSION['gst'] = $booking[14];
